...
def mostrar():
    ...
    if not st.session_state.get(base_key):
        ...
        # Unidades mezcladas: solo los productos con más de una unidad,
        # como listas ordenadas (se calcula acá, se lee en cada render).
        st.session_state[f"prov_unimix_v3_{semana}_{año}"] = {
            p: sorted(u) for p, u in _unidades_por_prod.items() if len(u) > 1}
    ...
    _mixtos     = st.session_state.get(f"prov_unimix_v3_{semana}_{año}", {})
    ...
    # confunde el consolidado. Se calcula al construir la demanda y se lee
    # desde session_state (persiste entre re-renders).
    if _mixtos:
